...
- **LLM service** (OpenAI) wired at startup, ready for conversation analysis
...
- **Entry point**: `app.py`
  ...
  - Bootstraps `LLM` service and registers routes
...
- The `LLM` service is initialized and available in `services["llm_service"]` but not exposed via routes in this repo.
